    ...
    for episode in range(3000):
        while True:
            ...
            if len(action_stack) > 0: #如果存放动作的栈不空，那么就从里面选择一个动作来执行
                ...
                action_button=env.choose_action(s_t) #根据这4帧图像选择动作,这个动作是自己设计的复合动作
                action_system=env.action_button_2_onehot(action_button)#np.append(np.append(a0,a1),b1)# #转变为系统所能是被的动作
                for i in range(3):
            ...
            env.store_memory(s_t, action_button, reward, s_t1, done) #存的时候还是存自己设计的复合动作按键
            
            if (total_steps > 3000) and (total_steps%100==0): #在步数超过200补之后开始学习，并且每50步学习一次
